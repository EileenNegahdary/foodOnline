...
def registerUser(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Your account has been registered successfully. Activation link has been sent to your emial.')
...
def activate(request, uidb64, token):
    try:
        ...
        print('uid value in compare token with user function',uid)
        user = User._default_manager.get(pk=uid)
    except(TypeError, ValueError, OverflowError, User.DoesNotExist):
        user = None
    
    # compare token with user
...
def forgot_password_validate(request, uidb64, token):
    ...
    # compare token with user
    if user is not None and default_token_generator.check_token(user, token):
        print('uid value in forgot password', uid)
